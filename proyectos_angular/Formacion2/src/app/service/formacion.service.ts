import { Injectable } from '@angular/core';
import {HttpClient, HttpHeaders} from '@angular/common/http';
import { Alumno } from '../model/Alumno';
import { Curso } from '../model/Curso';
import { Matricula } from '../model/Matricula';


@Injectable({
  providedIn: 'root'
})
export class FormacionService {

  urlAlumnos:string="http://localhost:8080/15_formacion_matriculas/Alumnos";
  urlCursos:string="http://localhost:8080/15_formacion_matriculas/Cursos";
  urlAlumnosCurso:string="http://localhost:8080/15_formacion_matriculas/AlumnosCurso";
  urlCursoAlumnos:string="http://localhost:8080/15_formacion_matriculas/CursoAlumnos";
  urlConsultarMatriculas: string =
    'http://localhost:8080/15_formacion_matriculas/Matriculas';
  urlMatricularAlumno: string =
    'http://localhost:8080/15_formacion_matriculas/Matricular';


  cursos:string[]|undefined;

  constructor(private http:HttpClient) {}
   
  listarCursos() {
  
      
    return this.http.get<Curso[]>(this.urlCursos);
  }

  listarAlumnos() {

    
    return this.http.get<Alumno[]>(this.urlAlumnos);
  }


  buscarAlumnosCurso(curso:string) {

    // El tipo indicado entre los diamantes, es el tipo de salida.
    return this.http.get<Alumno[]>(this.urlAlumnosCurso,{params:{nombre:curso}});
  }

  buscarCursosAlumno(usuario:string) {

    // El tipo indicado entre los diamantes, es el tipo de salida.
    return this.http.get<Curso[]>(this.urlAlumnosCurso,{params:{usuario:usuario}});
  }

  matriculas(fechaIni: string, fechaFin: string) {
    return this.http.get<Matricula[]>(this.urlConsultarMatriculas, {
      params: { fechaIni: fechaIni, fechaFin: fechaFin },
    });
}

  matricular(idCurso: number, usuario: string) {
  return this.http.post(`${this.urlMatricularAlumno}?idCurso=${idCurso}&usuario=${usuario}`,null)  
}

}

